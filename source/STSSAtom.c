...
static ErrInfo createDataFromFile(Atom* me, FileReader* fileReader)
{
	...
	myself->syncSampleTable = (uint*) malloc(sizeof(uint) * myself->numberOfEntries);
	for(i = 0; i < myself->numberOfEntries; i++)
	{
		READ_4BYTES(fileReader, myself, myself->syncSampleTable[i])
	}
...
ErrInfo CreateStssAtom(Atom** me, uint size, uint type)
{
	...
	myself = (StssAtom*) malloc(sizeof(StssAtom));
	SET_BASEATOM(myself);
	*me = (Atom*)myself;
	return err;
}
